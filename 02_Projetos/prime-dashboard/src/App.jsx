...
function KpiCard({ label, value, color }) {
  return (
    <div className="kpi-card">
      ...
      <span className="kpi-value" style={{ color: color ?? '#0f172a' }}>{value ?? '—'}</span>
    </div>
  )
}
...
function AgendamentosChart({ lineCards }) {
  ...
  const dIA    = groupByBucket(lineCards.cardsIA,   buckets)
  const dCRCA  = groupByBucket(lineCards.cardsCRCA, buckets)
  const dCRCB  = groupByBucket(lineCards.cardsCRCB, buckets)
  const dTotal = dIA.map((v, i) => v + dCRCA[i] + dCRCB[i])
  ...
  const chartData = {
    ...
    datasets: [
      ...
      },
      {
        ...
        data: dCRCA,
        borderColor: BLUE,
        backgroundColor: 'rgba(29,86,216,0.06)',
        borderWidth: 2,
        pointRadius: 3,
        pointHoverRadius: 5,
        fill: true,
  ...
  return (
    <div className="chart-card">
      <div className="chart-head line-head">
        <div>
          <p className="chart-title">Agendamentos por Canal</p>
          <p className="chart-subtitle">Evolução no período selecionado</p>
        </div>
...
export default function App() {
  ...
  const totalConsulta = (c.cFaltou ?? 0) + (c.cComp ?? 0) + (c.cFechou ?? 0)
  const totalComp     = (c.cComp ?? 0) + (c.cFechou ?? 0)
      ...
      <header className="header">
        <div className="brand">
          ...
          <div className="brand-text">
            <h1 className="brand-name">Prime Odontocenter</h1>
            <p className="brand-sub">Painel de Performance · IA + CRCs</p>
          </div>
        </div>
        ...
        <section>
          ...
          <div className="kpi-row">
            <KpiCard
              ...
              value={loading ? <Skeleton w={72} h={28} /> : totalLeads.toLocaleString('pt-BR')}
              color={BLUE}
            />
            <KpiCard
              label="Receita Fechada"
              value={loading ? <Skeleton w={110} h={28} /> : fmtBRL(data?.receita ?? 0)}
              color={GREEN}
            />
            <KpiCard
              label="Taxa de Comparecimento"
              value={loading ? <Skeleton w={56} h={28} /> : fmtPct(totalComp, totalConsulta)}
            />
            <KpiCard
              label="Taxa de Fechamento"
              value={loading ? <Skeleton w={56} h={28} /> : fmtPct(c.cFechou, totalComp)}
              color={GREEN}
            />
            <KpiCard
              label="Ticket Médio"
              value={loading ? <Skeleton w={100} h={28} /> : (c.cFechou ? fmtBRL((data?.receita ?? 0) / c.cFechou) : '—')}
            />
          </div>
        </section>
          ...
          {loading ? (
            ...
            <div className="donuts-row">
              <DonutChart
                ...
                subtitle="Não agendados · Meta vs Orgânico"
                labels={['Meta (pago)', 'Orgânico']}
                data={[c.cMeta ?? 0, c.cOrg ?? 0]}
                colors={[BLUE, BLUE2]}
              />
              <DonutChart
                title="Resultado das Consultas"
                subtitle="Desfecho após o agendamento"
                labels={['Faltou', 'Compareceu', 'Compareceu e Fechou']}
                data={[c.cFaltou ?? 0, c.cComp ?? 0, c.cFechou ?? 0]}
                colors={[RED, SLATE, GREEN]}
              />
              <DonutChart
                title="Conversão na Consulta"
                subtitle="Compareceu · Compareceu e Fechou"
                labels={['Compareceu', 'Compareceu e Fechou']}
                data={[c.cComp ?? 0, c.cFechou ?? 0]}
                colors={[SLATE, GREEN]}
              />
            </div>
          )}
