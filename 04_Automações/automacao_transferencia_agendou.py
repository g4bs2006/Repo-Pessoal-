# -*- coding: utf-8 -*-
"""
Automacao: transfere cards dos steps de "Agendados" do painel Prime Odontocenter
para o step "Agendou" do painel novo (Agendamentos), aplicando:
  - etiqueta de ORIGEM: Agendado pela IA / Agendado CRC A / Agendado CRC B
    (colunas "+ IA" recebem duas etiquetas de origem)
  - etiqueta Meta ou Organico conforme as tags do CONTATO vinculado ao card
    (se o contato nao tiver nenhuma das duas, o card entra so com a tag de origem)

Preserva a ordem das posicoes do painel antigo (por coluna, na sequencia abaixo).
Originais ficam intactos. Idempotente via metadata.sourceCardId.

Uso:
    python automacao_transferencia_agendou.py                # dry-run (so mostra)
    python automacao_transferencia_agendou.py --executar     # cria de fato
    python automacao_transferencia_agendou.py --executar --limite 1   # teste com 1
"""
import sys
import time
import json
...
STEP_AGENDOU = "b38ead0e-2328-4061-bf47-3aee414d11ec"

# Etiquetas (tags) do PAINEL destino
TAG_PANEL_IA = "18b8f717-2a64-478f-93d9-1da7d23adef2"        # Agendado pela IA
TAG_PANEL_CRC_A = "0fc9fce4-8068-4c4b-8f0e-a7cee9bcd0d4"     # Agendado CRC A
TAG_PANEL_CRC_B = "e246232e-cea5-4f53-b89d-2a6d28364a62"     # Agendado CRC B
...
# Steps de origem, na ordem em que serao inseridos no Agendou
ORIGENS = [
    {"nome": "Agendados pela IA",          "stepId": "096ecc8b-c84e-4f20-9df8-bc684c6dff45", "tagsOrigem": [TAG_PANEL_IA]},
    {"nome": "Agendados pela CRC A",       "stepId": "884ffe20-5cdb-4549-9301-07d38dc278a1", "tagsOrigem": [TAG_PANEL_CRC_A]},
    {"nome": "Agendados pela CRC B",       "stepId": "c86979eb-cb1f-43b4-aabf-f726a304d011", "tagsOrigem": [TAG_PANEL_CRC_B]},
    {"nome": "Agendamento pela CRC A + IA", "stepId": "0f85b8a1-10ac-47eb-b23a-c820457665e6", "tagsOrigem": [TAG_PANEL_CRC_A, TAG_PANEL_IA]},
    {"nome": "Agendamento pela CRC B + IA", "stepId": "1ac243f6-f6fe-44c2-906e-eb2d2b239612", "tagsOrigem": [TAG_PANEL_CRC_B, TAG_PANEL_IA]},
]
...
def main():
    ...
    print("Listando cards existentes no step Agendou do painel destino...")
    existentes = listar_todos(PAINEL_DESTINO, STEP_AGENDOU)
    ja_transferidos = set()
    for c in existentes:
    ...
    for origem in ORIGENS:
        ...
        for card in cards:
            ...
            tag_ids = list(origem["tagsOrigem"]) + ([tag_lead] if tag_lead else [])
    ...
    if sem_tag_meta_org:
        print(f"\nCards cujo contato NAO tinha tag Meta/Organico ({len(sem_tag_meta_org)}) - criados so com tag de origem:")
