...
> OBRIGATÓRIO: Acione esta habilidade ao avançar de estágio e sempre que o atendimento chegar a uma definição final. Nunca encerre o atendimento sem executá-la. Os momentos obrigatórios são: ao avançar de qualquer estágio; após agendamento confirmado no E5 ou E10 (após tag_Agendou e Cliente Agendou - IA); após remarcação confirmada no E6 (após tag_Remarcou); após cancelamento confirmado no E6 (após tag_Cancelou); e no E8 antes de concluir_atendimento. O campo 'text' (Notas Internas) deve conter a linha de 7 campos no formato definido no E11: [ESTÁGIO] [NOME] [DOR] [URGÊNCIA] [AGENDAMENTO] [ÚLTIMA_MENSAGEM] [PRÓXIMA_AÇÃO], seguida de um parágrafo iniciado por "Autoavaliação:". Mantenha os campos que não mudaram e atualize só o que evoluiu. Não encerre sem salvar. Não salve nota genérica.

**Descrição da Variável (`text` / Notas Internas):**
A LLM deve preencher o campo de texto seguindo rigorosamente o formato de 7 campos do E11, seguido da Autoavaliação:
...
| **ÚLTIMA_MENSAGEM** | Texto exato do último follow-up enviado (E12), ou `nenhuma`. |
...
> "[ESTÁGIO: E5] [NOME: Gabriel] [DOR: Estética — vergonha de sorrir há 5 anos] [URGÊNCIA: alta] [AGENDAMENTO: 25/04 às 14h — confirmado] [ÚLTIMA_MENSAGEM: nenhuma] [PRÓXIMA_AÇÃO: finalizar no E8; confirmar presença 24h antes; não reiniciar o funil]
> 
...
**Agendamento:**
`verificar_disponibilidade → Pacto de Honra → realizar_agendamento → tag_Agendou → Cliente Agendou - IA → Salvar_Contexto → E8`
