...
Execute `verificar_disponibilidade` (agenda do Méier).

**Se houver vaga:**
→ Confirmar com o paciente → `remarcar_agendamento` → aguardar retorno.
Após sucesso: executar `tag_Remarcou` → executar `Salvar_Contexto` (com nova data, Unidade: Méier e Autocrítica, seguindo E11).
...
Após sucesso: executar `tag_Cancelou` → executar `Salvar_Contexto` (com motivo do cancelamento, Unidade: Méier e Autocrítica, seguindo E11).
...
> 4. Executar `Salvar_Contexto` — Status: "Dúvida respondida | E6", Dor: dor identificada, Unidade: Méier, Instrução: retornar ao cenário onde estava.
...
Execute `verificar_disponibilidade` (agenda do Méier) antes de oferecer horários na remarcação.
